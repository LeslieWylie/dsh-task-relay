...
import { assertStringLength, assertPriority, assertTags } from './invariant.ts'
 ...
 */
export function registerTools(store: TaskRelayStore): ReturnType<typeof defineTool>[] {
  return [
    // ── task_push ──
    defineTool({
      name: 'task_push',
      description:
        '推送一条新任务到跨会话共享队列。同一 workspace 的所有会话（含子 agent）均可查看和认领。' +
        '返回新任务的完整信息。',
      parameters: {
      ...
      execute: async (args, _exec) => {
        await store.init()
        const { title, description = '', priority, tags = [] } = args as {
          title: string; description?: string; priority: string; tags?: string[]
        }
        assertStringLength(title, 200, 'title')
        if (description) assertStringLength(description, 4000, 'description')
    ...
    defineTool({
      ...
      execute: async (args) => {
        await store.init()
        const { status, priority, tags, limit } = args as {
          status?: string; priority?: string; tags?: string[]; limit?: number
        }
    ...
    defineTool({
      name: 'task_claim',
      description:
        '认领一个开放任务。认领后该任务标记为 claimed，其他会话不可重复认领。' +
        '返回认领后的任务信息。',
      parameters: {
    ...
    defineTool({
      name: 'task_done',
      description:
        '标记任务为已完成。需提供完成结果。认领者与当前会话不一致时禁止操作。' +
        '返回完成后的任务信息。',
      parameters: {
        id: { type: 'string', required: true, description: '任务 ID' },
    ...
    defineTool({
      name: 'task_cancel',
      description:
        '取消任务。如果任务是 open 状态则删除；如果已被认领则退回 open 状态。' +
    ...
    defineTool({
      name: 'handoff_write',
      description:
        '写入当前会话的交接摘要。用于会话结束时记录工作状态和待办事项，' +
    ...
    defineTool({
      name: 'handoff_read',
      description:
        '读取会话交接摘要。不指定 sessionId 时返回最近 N 条交接摘要（按时间降序）。' +
      ...
      execute: async (args) => {
        ...
        if (sessionId) {
          ...
          if (!h) return `未找到会话 ${sessionId} 的交接摘要`
