...
import { STORE_DIR, STORE_FILENAME } from './types.js';
/** 默认 store 数据。 */
function emptyStore() {
    return {
        tasks: {},
...
export class TaskRelayStore {
    ...
    listTasks(filter) {
        ...
        for (const id of this.data.orderedIds) {
            ...
            if (filter?.status !== undefined && t.status !== filter.status)
                continue;
            if (filter?.priority !== undefined && t.priority !== filter.priority)
                continue;
            if (filter?.session !== undefined && t.sourceSession !== filter.session && t.claimedBy !== filter.session)
                continue;
    ...
    /** 认领一个 open 任务。 */
    async claimTask(id, sessionId) {
        const t = this.data.tasks[id];
        if (!t)
            throw new Error(`task-relay: task "${id}" not found`);
        if (t.status !== 'open')
            throw new Error(`task-relay: task "${id}" is already ${t.status}`);
        const now = new Date().toISOString();
        t.status = 'claimed';
    ...
    /** 标记任务为已完成。 */
    async doneTask(id, result, sessionId) {
        const t = this.data.tasks[id];
        if (!t)
            throw new Error(`task-relay: task "${id}" not found`);
        if (t.status === 'done')
            throw new Error(`task-relay: task "${id}" is already done`);
    ...
    // ── 交接摘记操作 ──
    /** 写入或更新当前会话的交接摘记。 */
    async writeHandoff(sessionId, summary) {
        const now = new Date().toISOString();
